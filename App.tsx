  ...
  useEffect(() => {
    ...
    const unsubscribe = db.subscribeToAuth(async (user) => {
      if (user) {
        ...
        if (user.institutionId && user.institutionId !== 'squadran') {
          // Note: We don't auto-set currentInstitution here anymore to ensure they see the landing page first.
          // The institution will be set when they enter the code on the Unified Portal.
        }
        ...
        setCurrentUser(null);
        setIsSuperAdminMode(false);
        // Removed auto-restoration of institution from local storage to ensure landing page is shown first.
    ...
    // Auto-restoration of institution from local storage removed to ensure landing page is shown first as per user request.
                ...
                <>
                  ...
                  {loginTab === 'STUDENT' ? (
                    <div>
                      ...
                      {currentInstitution.emailDomain && (
                        <p className="text-[10px] text-slate-400 ml-2 mt-1 font-medium italic">Required: @{currentInstitution.emailDomain.replace(/^@/, '')}</p>
                      )}
                    </div>
                  ) : (
                    <>
              ...
              <div className="mt-4">
                ...
                {currentInstitution.emailDomain && (
                  <p className="text-[10px] text-slate-400 text-center mt-2 font-medium">
                    Please use your institution account (@{currentInstitution.emailDomain.replace(/^@/, '')})
                  </p>
                )}
              </div>
            )}
